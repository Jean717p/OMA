...
void Heuristic::solveFast(vector<double>& stat, int timeLimit) {
    ...
    for(tmp=0;tmp<nThread;tmp++){
        ...
        //cout << "Thread:" << tmp << '\n'; //tStart=clock();
        /**
         * La democratica al momento è under debug poiché genera ancora soluzioni non feasible
         * - questa versione lancia 4 thread a lavorare sulla cellbycell
         */
        if(tmp%2==0){ //"uno a te uno a me"
            //vThread[tmp]=std::thread(&Heuristic::democratic,this,tStart,outOfTime,dimDestArray,tmp,predefinedTaskCount*magicNumber,1);
            vThread[tmp]=std::thread(&Heuristic::cellByCell,this,tStart,outOfTime,dimDestArray,tmp,predefinedTaskCount*magicNumber,1);
        }
        else{
            vThread[tmp]=std::thread(&Heuristic::cellByCell,this,tStart,outOfTime,dimDestArray,tmp,predefinedTaskCount*magicNumber,1);
            //vThread[tmp]=std::thread(&Heuristic::democratic,this,tStart,outOfTime,dimDestArray,tmp,predefinedTaskCount*magicNumber,1);
    ...
    problem.objFunOfThreads[tmp]=INT32_MAX;
    //democratic(tStart,outOfTime,dimDestArray,tmp,predefinedTaskCount*magicNumber,0);
    cellByCell(tStart,outOfTime,dimDestArray,tmp,predefinedTaskCount,0);
    for(tmp=0;tmp<nThread;tmp++){
        vThread[tmp].join();
    }
    ...
    stat.push_back((double)bestTotalCost);
...
void Heuristic::cellByCell(double tStart,double outOfTime,int dimDestArray,int tId,int predefinedTaskCount,int cicloN){
    //Declaration & initialization
    //cout << "Thread "<< tId << " ci sono" << '\n';
    ...
    tmp=0;
    cellUtility = new CellDestData[1]; //Alloco una singola struct
    ...
    //cout << "thread allocazione riuscita : " << tId << '\n';
    //******************************************************ALGORITMO*************************************************
    while(clockCheck < outOfTime){ // Soglia tempo
        objFun=0;
    ...
    //cout << "Thread finito: " << tId << '\n';
    return;
}

void Heuristic::democratic(double tStart,double outOfTime,int dimDestArray,int tId,int predefinedTaskCount,int cicloN){
    //Declaration & initialization
    int tmp,tmp2,tmp3; //temporanee
    //cout << "Thread "<< tId << "ci sono" << '\n';
    double clockCheck = 0; //controllo dei sencondi rimasti
    ...
    while(clockCheck < outOfTime){ // Soglia tempo
        ...
        clockCheck=(double)(clock() - tStart) / CLOCKS_PER_SEC;
        if(clockCheck>outOfTime){
            return; //Il democratico è più lento, meglio controllare anche dopo il greedy
        }
